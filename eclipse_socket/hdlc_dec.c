...
void hdlc_dec_init(hdlc_dec *hdlc_decode)
{
    int i;
	...
	for (i = 0; i < sizeof(hdlc_decode->info); i++)
...
unsigned short compute_fcs(unsigned char *data, int length)
{
	...
	for (i = 0; i< length; i++)
	{
		fcs = (fcs >> 8) ^ fcstab[(fcs ^ ((unsigned short)*data)) & 0xff];
		data++;
	}
...
void hdlc_decode(hdlc_enc *hdlc_encode, hdlc_dec *hdlc_decode)
{
    ...
    {
		hdlc_decode->size = hdlc_encode->size;
		...
		hdlc_decode->fcs = compute_fcs(hdlc_decode->info, hdlc_decode->size);

		for (i = 0; i < hdlc_decode->size; i++)
		{
			if (hdlc_decode->info[i] == 0x7D)
			{
